...
import leaveGrp from "../assets/leaveGrp.png";
...
const Filterposts: FC<FilterpostsProps> = ({
  groupJoin,
  setGroupJoin,
  filter,
  setFilter,
  ...
  return (
    <div
      ...
      className="w-[calc(100vw-32px)] sticky bg-white top-12 z-20 lg:border-b mx-4 max-w-[1024px] flex justify-between items-center mb-4"
    >
      <p className="lg:hidden text-2xl font-bold text-black">Posts{filter==='All'?'(4)':'(1)'}</p>
      ...
      <div className="hidden lg:flex justify-around items-center gap-x-4">
        <p
          ...
          className={
            filter === "All"
              ? "py-5 border-b-2 border-black cursor-pointer"
              : "py-5 cursor-pointer"
          }
        >
          All Posts
      ...
      <div className="hidden lg:flex justify-around items-center mb-2 gap-x-4">
        ...
        <button
          onClick={() => {
            isAuthorized
              ? setGroupJoin((prev: boolean) => !prev)
              : setAuthPopup(true);
          }}
          className={
            groupJoin
              ? "bg-white flex items-center justify-around py-1.5 px-3 rounded text-[#616978] border border-[#616978]"
              : "bg-[#2F6CE5] flex items-center justify-around py-1.5 px-3 rounded text-white"
          }
        >
          {groupJoin ? (
            <Image src={leaveGrp} alt="Join Group" className="h-5 w-5 mr-1" />
          ) : (
            <Image
              src={joinGrouppng}
              alt="Join Group"
          ...
          {groupJoin ? "Leave Group" : "Join Group"}
        </button>
      </div>
    </div>
  );
